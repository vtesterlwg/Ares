...
@AllArgsConstructor
public final class EnableVanishItem implements CustomItem {
    ...
    public String getName() {
        return ChatColor.DARK_PURPLE + "Vanish";
    }
    ...
    public Runnable getRightClick(Player who) {
        return () -> {
            ...
            if (essentialsService.getVanishManager().isVanished(who)) {
                who.sendMessage(ChatColor.RED + "You are already vanished");
                return;
            }

            customItemService.getItem(DisableVanishItem.class).ifPresent(item -> who.getInventory().setItemInMainHand(item.getItem()));

            essentialsService.getVanishHandler().hidePlayer(who, true);
            who.sendMessage(ChatColor.YELLOW + "You are now " + ChatColor.RESET + "invisible");
        };
    }
}